...
class Question():
    ...
    async def eval(self, llm, n_per_question):
        ...
        answers, log_vals = sample(llm, conversations)
        df = pd.DataFrame([
...
def main(model, questions, n_per_question=1, output='eval_result.csv'):
    ...
    for question in questions:
        res, logs = asyncio.run(question.eval(llm, n_per_question))
        outputs.append(res)
